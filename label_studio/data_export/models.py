...
class DataExport(object):
    ...
    def save_export_files(project, now, get_args, data, md5, name):
        ...
        print("DataExport > save_export_files", project, now, get_args, md5, name)
        #print("DataExport > save_export_files", data)
        ...
        print("DataExport > save_export_files", filename_results, filename_info, annotation_number)
        try:
    ...
    def generate_export_file(project, tasks, output_format, download_resources, get_args, hostname=None):
        ...
        """
        print("DataExport > generate_export_file", project, output_format, download_resources, get_args, hostname, flush=True)
        #print("DataExport > generate_export_file", tasks, flush=True)
        ...
        with get_temp_dir() as tmp_dir:
            print("DataExport > generate_export_file", input_json, flush=True)
            converter.convert(input_json, tmp_dir, output_format, is_dir=False)
            files = get_all_files_from_dir(tmp_dir)
            print("DataExport > generate_export_file", files, flush=True)
