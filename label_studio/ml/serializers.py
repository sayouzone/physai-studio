...
class MLBackendSerializer(serializers.ModelSerializer):
    ...
    def validate_url(self, value):
        print("MLBackendSerializer > validate_url", value)
        if ".pt" in value:
            return value
    ...
    def _validate_healthcheck(self, attrs):
        print("MLBackendSerializer > _validate_healthcheck", attrs)
        healthcheck_response = MLBackend.healthcheck_(**attrs)
        print("MLBackendSerializer > _validate_healthcheck", healthcheck_response, type(healthcheck_response))

        # ✅ MLApiResult 전체 출력
        print("=== healthcheck_response ===")
        print("url         :", healthcheck_response.url)
        print("request     :", healthcheck_response.request)
        print("response    :", healthcheck_response.response)
        print("headers     :", healthcheck_response.headers)
        print("type        :", healthcheck_response.type)
        print("status_code :", healthcheck_response.status_code)
        print("is_error    :", healthcheck_response.is_error)
        print("error_msg   :", healthcheck_response.error_message)
        print("============================")
    ...
    def _validate_setup(self, attrs):
        print("MLBackendSerializer > _validate_setup", attrs)
        setup_response = MLBackend.setup_(**attrs)
        print("MLBackendSerializer > _validate_setup", setup_response, type(setup_response))
    ...
    def validate(self, attrs):
        print("MLBackendSerializer > validate", attrs)
