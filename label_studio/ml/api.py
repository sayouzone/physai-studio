...
class MLBackendDetailAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        print("MLBackendDetailAPI > get_object")
    
        print("Request method:", self.request.method)
        print("Request data:", self.request.data)        # DRF 파싱된 데이터 (JSON/Form 모두)
        print("Request body:", self.request.body)        # raw bytes
        print("Request headers:", self.request.headers)

        ml_backend = super(MLBackendDetailAPI, self).get_object()
        print("MLBackendDetailAPI > get_object", ml_backend)
        ml_backend.update_state(request=self.request)
        return ml_backend

    def perform_update(self, serializer):
        print("MLBackendDetailAPI > perform_update", serializer)
        ml_backend = serializer.save()
        ml_backend.update_state()
