...
- 내부 파라미터 K, D 는 등방 초점거리/DewarpData 기반으로 개선된
  sayou.image.metadata.estimate_intrinsics_from_metadata 를 사용.
- 레거시 DJIGeoreferencer 클래스 제거 (ImageMetadata 에 없는 필드 참조).

2026-07 (2차) — 투영 평면 정책 변경
- **LRF 를 프레임별 지면고도로 직접 쓰지 않는다.**
  nadir 촬영에서 LRF 조준점은 프레임마다 '행간 지면'과 '패널 상면'을
  번갈아 맞는다. 실측 사례에서 지면 명중 시 AGL 45.40 m, 패널 명중 시
  43.50 m → 프레임 간 스케일 편차 4.2%. 인접 사진끼리 같은 패널이
  어긋나고 격자 적합 파라미터가 프레임마다 튀는 원인이 된다.
- 대신 **사이트 단위로 고정된 패널면 절대고도**(SITE_PANEL_PLANE_ALT)를
  1순위로 사용한다. 투영 평면을 패널 상면에 두면
    (a) 절대 스케일이 맞고
    (b) 패널 높이로 인한 기복변위(relief displacement,
        d = h·r/H — 높이 1.9 m 기준 프레임 끝에서 약 70 cm,
        중심에서 0이고 바깥으로 선형 증가)까지 함께 사라진다.
- 어떤 평면이 쓰였는지 추적할 수 있도록 ground_plane_source 를 노출.
  RGB/IR 경로가 같은 평면을 공유하는지 검증할 때 필수.
"""
...
from sayou.image.metadata import ImageMetadata, estimate_intrinsics_from_metadata
...
def calibrate_plane_altitude(
    metadata: ImageMetadata,
    known_size_m: float,
    measured_size_px: float,
) -> dict:
    """
    알려진 물리 치수를 가진 객체(예: PV 모듈 폭)로부터
    투영 평면의 절대고도를 역산한다. 사이트당 1회만 하면 된다.

    Parameters
    ----------
    metadata          : 대상 이미지의 ImageMetadata
    known_size_m      : 객체의 실제 치수 (m). 예: 모듈 폭 0.992
    measured_size_px  : 같은 객체의 이미지상 평균 픽셀 치수.
                        (같은 축 방향으로! 폭이면 폭 픽셀)
    ...
    {
      "agl_m"            : 역산된 대지고도,
      "plane_altitude_m" : 투영 평면 절대고도 (→ SITE_PANEL_PLANE_ALT 에 넣을 값),
      "gsd_m_per_px"     : 해당 평면에서의 GSD,
      "panel_height_over_lrf_m": LRF 조준점 대비 평면 높이 (참고용),
    }

    사용 예
    -------
        m = extract_metadata(path)
        # anomaly(모듈) 박스 N개의 평균 픽셀 폭이 160.4 px 이고
        # 실제 모듈 폭이 992 mm 라면:
        calibrate_plane_altitude(m, 0.992, 160.4)
    """
...
class DJIImageGeoreferencer:
    ...
    def __init__(
        ...
        Parameters
        ----------
        ground_altitude      : 투영 평면 절대고도를 직접 지정 (최우선)
        panel_plane_altitude : 사이트 패널면 절대고도.
                               None 이면 모듈 상수 SITE_PANEL_PLANE_ALT 사용.
        prefer_lrf           : True 로 두면 사이트 평면이 없을 때 LRF 를 쓴다.
                               기본 False — LRF 는 프레임마다 지면/패널을
                               번갈아 맞아 스케일이 4% 수준으로 흔들리므로
                               다중 프레임 정합에는 권장하지 않는다.
                               (단일 프레임 절대정확도만 볼 땐 켜도 됨)
        """
        ...
        # ── 투영 평면 절대고도 결정 ────────────────────────────────
        # 우선순위: 명시값 > 사이트 패널면 > (옵션)LRF > AGL 폴백
        #
        ...
        if ground_altitude is not None:
            ...
            self.ground_plane_source = GroundPlaneSource.EXPLICIT
        elif site_plane is not None:
            # 권장 경로: 사이트 전체가 동일 평면을 공유 → 프레임 간 스케일 일관
            self.ground_altitude = site_plane
            self.ground_plane_source = GroundPlaneSource.SITE_PLANE
        elif prefer_lrf and metadata.has_valid_lrf:
            self.ground_altitude = metadata.lrf_target_abs_alt
            self.ground_plane_source = GroundPlaneSource.LRF
            logger.warning(
                "[DJIImageGeoreferencer] LRF 기반 지면고도 사용 "
                "(abs_alt=%.3f). 조준점이 행간 지면/패널 상면 중 무엇을 "
                "맞았는지에 따라 프레임 간 스케일이 수 %% 흔들릴 수 있습니다. "
                "다중 프레임 정합에는 SITE_PANEL_PLANE_ALT 사용을 권장합니다.",
                metadata.lrf_target_abs_alt,
            )
        else:
            # 폴백: 절대고도 - 상대고도(이륙지점 기준 AGL)
            # 주의: relative_height 는 '이륙지점 지면' 기준이므로
            #       패널 상면보다 낮은 평면을 잡는다 → 스케일이 과대평가된다.
            self.ground_altitude = metadata.gps.altitude - metadata.relative_height
            self.ground_plane_source = GroundPlaneSource.REL_HEIGHT
            logger.info(
                "[DJIImageGeoreferencer] 사이트 패널면 평면이 설정되지 않아 "
                "relative_height 폴백 사용 (AGL=%.3f). 검출 대상이 패널 상면이면 "
                "스케일이 수 %% 과대평가됩니다. calibrate_plane_altitude() 로 "
                "SITE_PANEL_PLANE_ALT 를 설정하세요.",
        ...
        if self.ground_up_in_enu >= 0:
            raise ValueError(
                ...
                f"source={self.ground_plane_source}). 평면 고도 설정을 확인하세요."
            )
    ...
    def relief_displacement_m(self, object_height_m: float, radius_m: float) -> float:
        ...
        평면을 패널 상면에 맞추면 패널에 대해서는 h≈0 이 되어 사라진다.
        평면이 지면에 있고 패널이 h 만큼 높으면 프레임 바깥으로 갈수록
        선형으로 커지는 오차가 남는다. (진단용)
        """
    ...
    def validate_with_lrf(self) -> Optional[dict]:
        ...
        vertical_gap_m 으로 어느 쪽인지 가늠할 수 있다
        (패널면 평면을 쓸 때 이 값이 +1~2 m 면 LRF 가 행간 지면을,
         0 근처면 패널 상면을 맞은 것).
        """
