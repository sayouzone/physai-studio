...
배치 위치
---------
``sayou/georeferencing/homography/`` 를 가정한다 (``..geometry``, ``..crs``,
...
::
    ...
    from sayou.georeferencing.homography import (
        PipelineConfig, RTKHomographyPipeline,
    )

    pipe = RTKHomographyPipeline(PipelineConfig(target_epsg=5186))
    stats = pipe.run_direct(metas, "ortho.tif")   # SfM 없이 RTK 자세만으로

BA 를 거치려면 ``prepare`` → ``select_pairs`` → (기존 SfM 매칭/track/삼각측량)
→ ``bundle_adjust`` → ``build_homographies`` → ``ortho.mosaic_frames`` 순서로
호출한다. ``pipeline`` 모듈 docstring 참조.
...
from .pipeline import FrameContext, PipelineConfig, RTKHomographyPipeline
