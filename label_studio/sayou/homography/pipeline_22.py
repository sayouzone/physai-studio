...
def run_homography_pipeline(image_dir: Path,
                 ...
                 two_layer_auto: bool = True,
                 two_layer_max_shift_px: float = 10.0,
    ...
    if not skip_sfm:
        ...
        # ★ 반복 격자에서의 '한 줄 건너뛴' 오매칭을 RTK 로 거른다.
        #
        #   실측(EWP-서오창IC-2 IR 원본 10장): 줄무늬 주기가 **이미지 폭의
        #   25%**(한 장에 4줄)라 SIFT 가 옆 줄을 같은 줄로 착각하기 쉽다.
        #   실제로 쌍별 변위를 재니 주기의 정수배(-2.06, -1.19, 2.18, -2.39,
        #   -2.17, -1.96, 2.19 …)에 몰렸고, **10쌍 중 7쌍이 정수배 ±0.25
        #   이내**였다 (무작위면 50% 인데 70%).
        #
        #   이런 오매칭은 **기하학적으로 자기일관적**이라 RANSAC 도 BA 도
        #   못 거른다. BA 재투영 RMSE 가 0.74 px 로 좋았는데도 모자이크가
        #   찢어진 이유가 바로 이것이다.
        #
        #   RTK 는 위치를 cm 급으로 알므로, 행 간격(수 m)만큼 어긋난 대응은
        #   즉시 모순이 된다. `guided_match` 모듈이 이미 있었는데 파이프라인
        #   에서 **한 번도 호출되지 않고 있었다.**
        if guided_matching:
            try:
                from .homography.guided_match import guided_build_tie_points
                ...
                for _i, (_m, _k) in enumerate(zip(metas, intrinsics_obj)):
                    ...
                    _prov.append(build_frame_homography(_C, _R, _k, _plane0))
                # ★ 탐색 반경은 **줄무늬 주기의 절반보다 작아야** 한다.
                #   기본 200 px 는 실측 주기 160 px 보다 커서 '한 줄 건너뛴'
                #   대응이 그대로 반경 안에 들어온다 — 유도의 의미가 없다.
                #   원본 한 장에서 주기를 재서 자동으로 정한다.
                _radius = guided_search_px
                if guided_search_px <= 0:
                    _pitch = _estimate_stripe_pitch_px(metas)
                    if _pitch:
                        _radius = max(0.4 * _pitch, 20.0)
                        logger.info(
                            "RTK 유도 탐색반경 자동 결정: %.0f px "
                            "(줄무늬 주기 %.0f px 의 40%%). 주기의 절반을 "
                            "넘으면 '한 줄 건너뛴' 대응이 걸러지지 않습니다.",
                            _radius, _pitch)
                    else:
                        _radius = 200.0
                if all(f is not None for f in _prov):
                    _gm = guided_build_tie_points(
                        metas, pairs, _prov, features,
                        search_radius_px=_radius)
                    if len(_gm) >= 0.3 * max(len(matches), 1):
                        logger.info(
                            "RTK 유도 매칭으로 교체: 쌍 %d → %d "
                            "(반복 격자의 '한 줄 건너뛴' 오매칭을 거릅니다)",
                            len(matches), len(_gm))
                        matches = _gm
                    else:
                        logger.warning(
                            "RTK 유도 매칭 결과가 너무 적어(%d/%d 쌍) 기존 "
                            "매칭을 유지합니다 — --guided-search-px 를 "
                            "늘려보세요.", len(_gm), len(matches))
            except Exception as exc:
                logger.warning("RTK 유도 매칭 실패 (기존 매칭 유지): %s", exc)
    ...
    # ★ 열화상에서는 노출 보정을 끕니다.
    #   RGB 에서 프레임 간 밝기 차이를 없애는 것은 올바른 처리지만,
    #   **열화상에서 밝기는 곧 온도**입니다. 프레임마다 이득을 곱하면
    #   온도값이 프레임마다 다르게 스케일됩니다.
    #
    #   실측(그린환경센터 IR): 이득 범위 0.876~1.151 = 최대 1.31배 차이.
    #   픽셀값이 온도에 선형이라면 주변 대비 5°C 인 핫스팟이 시임 건너편에서
    #   3.8~6.6°C 로 보일 수 있습니다. **핫스팟 임계를 절대 온도차로 잡으면
    #   시임 건너편에서 판정이 뒤집힙니다** — 결함 판독에 직접 해롭습니다.
    #
    #   파일명 `_T` 접미사(DJI H20T 열화상)로 판별합니다.
    if exposure_compensate and thermal_auto:
        # ★ `keep` 은 이 지점에 이르는 경로에 따라 정수 인덱스 리스트이거나
        #   (frame, meta) 쌍이거나 달라 언패킹이 깨졌다 (TypeError 발생).
        #   `frame_metas` 는 787행에서 이미 만들어진, 이 시점의 진짜 대상
        #   목록이므로 그것을 직접 쓴다.
        _n_t = sum(1 for m in frame_metas
                   if Path(m.origin_path).stem.endswith("_T"))
        if _n_t >= 0.5 * max(len(frame_metas), 1):
            logger.info(
                "열화상 감지 (%d/%d 장이 _T) — 노출 보정을 끕니다. "
                "열화상에서 밝기는 온도이므로 프레임별 이득을 곱하면 "
                "핫스팟 판정이 시임 건너편에서 달라집니다. "
                "(--force-exposure 로 강제 사용 가능)", _n_t, len(frame_metas))
            exposure_compensate = False

    # ★ 2층을 쓸지는 **센서에 따라 다릅니다.** 실측(그린환경센터, 동일 QC):
    #     RGB  2층 켬  중앙값 0.0559, 단차99% 0.2912, 3px초과 6.3%
    #     RGB  2층 끔  중앙값 0.0513, 단차99% 0.2718, 3px초과 5.9%  ← 끄는 게 나음
    #     IR   2층 켬  중앙값 0.0416 (= 1.0 px, 측정 바닥)          ← 켜서 최고
    #
    #   2층 경계는 **단일 평면 정사영상**에서 따므로 이미 Δh·k 만큼 밀려
    #   있습니다. 그 밀림이 GSD 대비 몇 픽셀인지가 톱니의 크기입니다.
    #     RGB(GSD 0.64cm): k=0.15 에서 23 px → 눈에 보임
    #     IR (GSD 4.15cm): k=0.15 에서 3.6 px → 부화소급
    #
    #   ★ 이 판정은 **GSD 가 정해진 뒤에** 해야 합니다. 처음에 이 블록을
    #     GSD 자동 결정보다 앞에 두었다가 두 가지 버그를 냈습니다:
    #       (1) gsd_m 이 None 이라 로그의 %.4f 에서 TypeError (실행 중단)
    #       (2) None 을 0.01 로 대체하니 IR 도 15 px 로 계산돼 **잘못 해제**
    if use_two_layer and two_layer_auto and dsm_obj is None:
        _k_typ = max(float(max_offnadir_ratio), 0.05)
        _dh = max(float(_ground_drop_hint), 0.3)
        _shift_px = _dh * _k_typ / max(float(gsd_m), 1e-6)
        if _shift_px > two_layer_max_shift_px:
            logger.info(
                "2층 표면 자동 해제: 층 경계가 GSD 기준 %.0f px 밀려 있어 "
                "(층 높이차 %.2f m × k %.2f ÷ GSD %.4f m) 톱니 손해가 이득을 "
                "넘어섭니다. 한계 %.0f px. GSD 가 거친 IR 에서는 켜집니다.",
                _shift_px, _dh, _k_typ, float(gsd_m), two_layer_max_shift_px)
            use_two_layer = False
            _tl_builder = None      # ★ 이미 만들어진 빌더를 실제로 무효화한다
        else:
            logger.info("2층 표면 사용: 층 경계 밀림이 GSD 기준 %.1f px 로 "
                        "작습니다 (한계 %.0f px)",
                        _shift_px, two_layer_max_shift_px)
    # ---- 초점거리 최종 검증 (GSD 기반) --------------------------------
    ...
    # ★ 지형면 적합은 GroundPlane 전용 로그(위 '평면 경사 %.3f°')가
    #   끝난 뒤에 적용한다. 처음엔 위치가 잘못돼 plane 을 여기서
    #   PolyTerrain 으로 바꾼 뒤에도 아래에서 plane.slope_deg 등
    #   GroundPlane 전용 속성을 계속 읽어 AttributeError 로 죽었다.
    # ★ 경사지에서는 단일 평면이 무너집니다.
    #   실측(EWP-서오창IC-2, 경사지): IR 모자이크 가운데에서 패널 행이
    #   크게 찢어졌습니다 — 행 주기 6.19 m 인데 상단 경계 점프가 99% 2.01 m,
    #   최대 5.82 m, 8개 행이 44조각으로 끊김. 패널 높이(1.5 m)로는 k=0.9
    #   여도 1.35 m 라 설명이 안 되고, **경사 지형의 국소 기복**(Δh≈5 m)에
    #   큰 k(연직 제한 예외 37.2%)가 곱해진 결과입니다.
    #
    #   DSM 은 셀당 점이 부족해 실패하고(build_failed), 2층 모델은 지면이
    #   계속 변하는 경사지에서 가정이 깨집니다. 저차 다항면은 **전역 적합**
    #   이라 점이 뭉쳐 있어도 성립합니다 (계수 6개 대 점 수만 개).
    #
    ...
    if mosaic:
        ...
        stats = mosaic_frames(frames, [m.origin_path for m in frame_metas],
                              ...
                              cfg=MosaicConfig(
                                  ...
                                  panel_unit_m=panel_unit_m,
                                  panel_unit_k_mult=panel_unit_k_mult,
                                  panel_unit_glint_weight=panel_unit_glint_weight,
    ...
    summary = {
        ...
        # ★ terrain_fit 이 채택되면 plane 이 PolyTerrain 이라 GroundPlane
        #   전용 속성(a/b/c/slope_deg/...)이 없다. AttributeError 로
        #   전체 실행이 죽었던 지점 — 타입을 보고 분기한다.
        "ground_plane": (
            {
            ...
            if hasattr(plane, "a") else
            {"type": "poly_terrain", "degree": getattr(plane, "degree", None),
             "coef": (plane.coef.tolist() if hasattr(plane, "coef") else None)}
        ),
        "ortho": stats,
    }
