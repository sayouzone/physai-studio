...
# ---------------------------------------------------------------------------
class FrameSet:
    """프레임 집합의 준비 → 품질게이트 → 평면 → 호모그래피를 담당.

    상위 ``georeferencing.pipeline.run_pipeline`` 이 배치 실행을 맡고, 이
    클래스는 라이브러리 수준 API (검출 결과 지오코딩 등) 로 쓴다.
    """

    def __init__(self, config: FrameSetConfig | None = None):
        self.cfg = config or FrameSetConfig()
