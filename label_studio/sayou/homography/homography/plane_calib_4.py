...
def estimate_plane_height_from_overlaps(
    ...
    *,
    ...
    # ★ 기각하더라도 **추정의 일관성(쌍별 산포)을 함께 남긴다.**
    #   이전에는 크기 검사가 먼저 걸려 산포를 볼 수 없었다. 극동대에서
    #   Δz = -4.50 / -5.85 / -6.24 m 가 세 데이터에서 나왔는데, 그것이
    #   '측정이 일관되게 평면이 높다고 말하는 것' 인지 '겹침이 엉망이라
    #   값이 튄 것' 인지 로그만으로는 구분할 수 없었다.
    #
    #   산포가 작으면 추정 자체는 믿을 만하다는 뜻이므로, 그때는
    #   --plane-shift-limit 을 올려 시험해 볼 가치가 있다. 기본 한계는
    #   그대로 둔다 — 과거 그린환경센터에서 -8.2 m 가 통과해 기준면이
    #   지면 아래로 내려가 결과를 망친 적이 있다.
    limit = max_shift_m
    if abs(dz) > limit:
        _supported = resid < 0.15 * abs(dz)
        logger.warning(
            "평면 높이 자동보정 기각: Δz=%+.2f m 가 타당 범위 ±%.1f m 를 "
            "벗어났습니다 (카메라-평면 %.1f m, 쌍 %d/%d, 산포 ±%.2f m). "
            "%s",
            dz, limit, cam_h, n_used, len(cand), resid,
            ("산포가 추정값의 %.0f%% 로 작습니다. **다만 일관성은 비편향을 "
             "뜻하지 않습니다** — 실측(극동대)에서 24쌍 중 23쌍이 ±0.41 m 로 "
             "일치하며 -6.24 m 를 가리켰지만, 적용하니 패널 어긋남이 "
             "0.666 → 0.798 m 로 **나빠졌습니다** (Zoom 도 0.549 → 0.751 m). "
             "연직 관측이 없고(최소 k 중앙값 0.17~0.31) 지면·패널 두 층이 "
             "섞이면 모든 쌍이 같은 방향으로 치우쳐 서로 일치하면서 다 같이 "
             "틀립니다. 시험하시려면 --plane-shift-limit 과 "
             "--plane-lrf-tolerance 를 %.0f 로 함께 주되, 반드시 "
             "quality.panel_misalign_median_m 로 확인하세요."
             % (resid / max(abs(dz), 1e-6) * 100, abs(dz) + 1)
             if _supported else
             "산포가 추정값의 %.0f%% 로 커서 추정을 신뢰할 수 없습니다 — "
             "겹침에 시차 외 불일치가 많다는 뜻입니다."
             % (resid / max(abs(dz), 1e-6) * 100)))
        return None
