...
def build_layer_surface(points_xyz: np.ndarray,
                        bounds: tuple,
                        plane,
                        layer_gap_m: float,
                        *,
                        ...
                        assign_band_m: float = 0.35,
                        smooth_cells: float = 0.6,
                        min_coverage: float = 0.20) -> "DSM | None":
    """**2층 표면 모델** — 연속 DSM 이 실패하는 곳에서 쓰는 대안.

    왜 이것이 되는가
    ----------------
    연속 DSM 은 셀마다 높이를 *로버스트하게 추정*해야 해서 중앙값+MAD 에
    필요한 셀당 10개 이상을 못 채웠습니다 (실측 충전율 26.6%). 점이 텍스처
    있는 곳에 뭉치기 때문입니다.

    그런데 **이 장면의 높이는 연속이 아닙니다.** 지면(113.1 m) 아니면
    패널 상면(114.0 m), 둘 중 하나입니다. 그러면 필요한 것은 높이 추정이
    아니라 **이진 분류**이고, 다수결은 셀당 3~5개면 성립합니다.
    ...
    왜 이것이 중요한가 (실측)
    ------------------------
    현재 병목은 BA 가 아니라 단일 평면입니다:

    * 정밀 단계 재투영 잔차 중앙값 2.11 px = 지상 **1.4 cm**
    * 그런데 모자이크 안쪽 어긋남은 **3.9 cm**

    차이는 기준면과 실제 표면의 높이차 ``Δh`` 가 만드는 기복변위 ``Δh·k``
    입니다. 기준면이 패널 상면에 있으므로 **지면층이 0.96 m 아래**에 있고,
    ``k=0.15`` 에서 **14 cm**, ``k=0.304`` 에서 **29 cm** 밀립니다.

    2층 모델은 이 항을 없앱니다. 남는 것은 층 내부 산포(0.27 m)뿐이고,
    그건 ``k=0.15`` 에서 4 cm 입니다.

    Parameters
    ----------
    plane : 상층(패널 상면) 기준면.
    layer_gap_m : 하층이 상층보다 얼마나 아래인지 (양수).
    assign_band_m : 점을 층에 배정할 때 각 층 중심에서 허용하는 반폭.
        두 층 사이 애매한 점은 버립니다.
    """
    pts = np.asarray(points_xyz, dtype=np.float64)
    pts = pts[np.all(np.isfinite(pts), axis=1)]
    if len(pts) < 500 or layer_gap_m <= 0.05:
        return None
    ...
    z_plane = plane.height_at(pts[:, 0], pts[:, 1])
    resid = pts[:, 2] - z_plane
    upper = np.abs(resid) <= assign_band_m
    lower = np.abs(resid + layer_gap_m) <= assign_band_m
    use = upper | lower
    if use.sum() < 500:
        logger.warning("2층 모델: 층에 배정된 점이 %d개로 부족", int(use.sum()))
        return None

    j = ((pts[use, 0] - x_min) / cell_m).astype(np.int64)
    i = ((y_max - pts[use, 1]) / cell_m).astype(np.int64)
    is_up = upper[use]
    ok = (i >= 0) & (i < ny) & (j >= 0) & (j < nx)
    i, j, is_up = i[ok], j[ok], is_up[ok]
    flat = i * nx + j

    n_up = np.bincount(flat[is_up], minlength=ny * nx)
    n_lo = np.bincount(flat[~is_up], minlength=ny * nx)
    total = n_up + n_lo
    known = total >= min_pts_per_cell
    coverage = float(known.mean())

    logger.info("2층 표면: 셀 %.2f m, 상층 %d점 / 하층 %d점, "
                "판정된 셀 %.1f%% (층 간격 %.2f m)",
                cell_m, int(is_up.sum()), int((~is_up).sum()),
                coverage * 100, layer_gap_m)
    if coverage < min_coverage:
        logger.warning("2층 표면 폐기: 판정된 셀 %.1f%% < %.0f%%",
                       coverage * 100, min_coverage * 100)
        return None

    # 다수결 → 상층이면 0, 하층이면 −gap.
    off = np.where(n_up >= n_lo, 0.0, -layer_gap_m).reshape(ny, nx)
    known2 = known.reshape(ny, nx)

    # 빈 셀은 주변 다수결로 메운다 (이진이라 팽창이 안전하다).
    up_c = (n_up.reshape(ny, nx) * known2).astype(np.float32)
    lo_c = (n_lo.reshape(ny, nx) * known2).astype(np.float32)
    for _ in range(8):
        if known2.all():
            break
        k = np.ones((3, 3), np.float32)
        us = cv2.filter2D(up_c, -1, k, borderType=cv2.BORDER_REPLICATE)
        ls = cv2.filter2D(lo_c, -1, k, borderType=cv2.BORDER_REPLICATE)
        new = (~known2) & ((us + ls) > 0)
        off[new] = np.where(us[new] >= ls[new], 0.0, -layer_gap_m)
        up_c[new] = us[new]; lo_c[new] = ls[new]
        known2 |= new
    off[~known2] = 0.0
    ...
    X, Y = np.meshgrid(xs, ys)
    height = plane.height_at(X, Y) + off

    if smooth_cells > 0:
        height = cv2.GaussianBlur(height.astype(np.float32), (0, 0),
                                  smooth_cells)

    dsm = DSM(height.astype(np.float32), x_min, y_max, cell_m,
              int(use.sum()), coverage)
    st = dsm.stats()
    logger.info("2층 표면 생성: %d×%d 셀, 표고 %.2f~%.2f m (기복 %.2f m) — "
                "지면층 기복변위(%.2f m × k)를 제거합니다",
                ny, nx, st["z_min"], st["z_max"], st["relief_p99_m"],
                layer_gap_m)
    return dsm
