...
class MosaicConfig:
    ...
    def __init__(self,
        ...
        self.offnadir_fallback_steps = int(max(offnadir_fallback_steps, 1))
...
def mosaic_frames(frames: list[FrameHomography],
    ...
    # ★ 예외를 **단계적으로** 적용한다.
    #   이전에는 k≤0.15 로 못 덮은 픽셀을 곧바로 제한 없이(k 최대 0.467)
    #   채웠다. 한 번에 3배를 뛰므로 그 영역의 어긋남이 0.146 → 0.427 m 로
    #   튀었다 (실측). 예외 영역은 가장자리 링이지만 **패널의 34.8% 가
    #   거기에 있어** 버릴 수도 없었다.
    #
    #   대신 제한을 조금씩 풀어 가며 **각 픽셀이 쓸 수 있는 가장 작은 k**
    #   로 채운다. 안쪽은 그대로 k≤0.15 를 쓰고, 가장자리는 0.20 → 0.25 →
    #   … 순으로 필요한 만큼만 열린다. 절벽 대신 완만한 저하가 된다.
    n_fallback = 0.0
    ...
    if (cfg.max_offnadir_ratio > 0 and cfg.offnadir_fallback
        ...
        k_max_frame = 0.0
        for fh in frames:
            k_max_frame = max(k_max_frame, float(
                np.hypot(fh.intr.width, fh.intr.height)
                / 2.0 / max(fh.intr.f_px, 1e-6)))
        base = cfg.max_offnadir_ratio
        n_steps = max(int(cfg.offnadir_fallback_steps), 1)
        if k_max_frame > base and n_steps > 1:
            levels = list(np.linspace(base, k_max_frame, n_steps + 1))[1:]
        else:
            levels = [k_max_frame if k_max_frame > base else 0.0]

        saved = cfg.max_offnadir_ratio
        for lv in levels:
            hole = label_map < 0
            if not hole.any():
                break
            cfg.max_offnadir_ratio = float(lv)
            _WEIGHT_CACHE.clear()
            _, lab_lv, _, _, _ = _coarse_reference(
                frames, image_paths, order, bounds, cfg,
                restrict=(lv < k_max_frame - 1e-6))
            if lab_lv is None:
                continue
            fill = hole & (lab_lv >= 0)
            if fill.any():
                label_map[fill] = lab_lv[fill]
                fallback_levels.append((float(lv), float(fill.mean())))
        cfg.max_offnadir_ratio = saved
        _WEIGHT_CACHE.clear()
        n_fallback = float(sum(f for _, f in fallback_levels))
        if fallback_levels:
            logger.info("연직 제한 단계적 예외: " + ", ".join(
                f"k≤{lv:.2f} 로 {fr*100:.1f}%" for lv, fr in fallback_levels)
                + f" (합계 {n_fallback*100:.1f}%). 한 번에 최대 k 로 "
                  "뛰지 않으므로 가장자리 열화가 완만합니다.")
    ...
    return {
        ...
        "offnadir_fallback_levels": fallback_levels,
