...
class MosaicConfig:
    ...
    seam_blend_px : ``"select"`` 모드에서 시임 좌우 이 폭(출력 픽셀)만
        부드럽게 섞는다. ``0`` (기본) 이면 하드 컷. 노출 차이로 이음선이
        눈에 띌 때만 20~60 정도를 준다. **크게 주면 그 밴드 안에서 다시
        ghosting 이 생기므로** 이음선이 실제로 거슬릴 때만 최소로 쓸 것.
        패널이 시임에서 어긋나는 문제는 이 값으로 해결되지 않는다 —
        그건 기준 평면 높이 문제다 (``plane.estimate_ground_plane`` 의
        ``surface="upper"`` 참조).
    ...
    for fh, path in zip(frames, image_paths):
        ...
        if select_mode:
            ...
            if cfg.seam_blend_px > 0:
                # 시임 근처(두 점수가 비슷한 좁은 띠)에서만 선형 혼합.
                # 점수차를 폭으로 정규화해 0.5 를 중심으로 한 알파를 만든다.
                # 밴드 밖은 알파가 0/1 로 포화 → 하드 선택과 동일.
                occupied = score_sub > 0
                span = cfg.seam_blend_px * float(np.max(weight) + 1e-6) / max(w, h)
                span = max(span, 1e-6)
                alpha = np.clip(0.5 + 0.5 * (weight - score_sub) / span, 0.0, 1.0)
                alpha[~occupied] = np.where(weight[~occupied] > 0, 1.0, 0.0)
                alpha[weight <= 0] = 0.0
                mix = alpha[:, :, None]
                blended = (warped[:, :, :bands].astype(np.float32) * mix
                           + canvas_sub.astype(np.float32) * (1.0 - mix))
                touched = alpha > 0
                canvas_sub[touched] = blended[touched].astype(cfg.dtype)
