...
def _unit_glint(fh, images, idx: int, x_min: float, y_max: float, g: float,
                u0: int, v0: int, u1: int, v1: int,
                thresh: int, img_scale: float = 1.0) -> float | None:
    """후보 프레임을 이 단위에 워프했을 때의 **포화 비율**.

    ``img_scale`` 은 ``images`` 가 원본의 몇 배로 축소돼 있는지 (저해상도
    패스는 0.25 를 쓴다). 호모그래피에 같은 배율을 곱해야 좌표가 맞는다.
    """
    im = images.get(idx) if hasattr(images, "get") else None
    if im is None:
        return None
    nx = u1 - u0 + 1; ny = v1 - v0 + 1
    if nx < 4 or ny < 4:
        return None
    H = fh.ortho_pixel_matrix(x_min + u0 * g, y_max - v0 * g, g)
    if img_scale != 1.0:
        S = np.diag([img_scale, img_scale, 1.0]).astype(np.float64)
        H = S @ H
            ...
            for u0 in range(cx0, cx0 + cw, step):
                ...
                for idx in order[:8]:
                    ...
                    if kk is None:
                        continue
                    cost = kk
                    if glint_weight > 0 and images is not None:
                        # ★ "그 프레임이 이 단위 **전체** 를 칠했다면" 의
                        #   포화도를 재야 한다.
                        #
                        #   처음에는 그 프레임이 **이미 칠한 픽셀**로만 쟀는데,
                        #   그 픽셀들은 시임 경쟁에서 이긴 곳이라 표본이
                        #   편향됩니다. 시임은 |새 프레임 − 캔버스| 가 작은
                        #   곳을 고르는데, 포화된 영역은 255 로 뭉개져 차이가
                        #   **오히려 작게** 나오므로 포화 프레임이 그 자리를
                        #   이깁니다. 그 결과 실측에서 포화가 5.01% → 5.27%
                        #   로 **늘었습니다** (250+ 는 +14.5%).
                        #
                        #   워프 기반으로 고치니 합성에서는 포화가
                        #   16.2% → 7.5% 로 줄었습니다. 그런데 **실데이터에서는
                        #   5.01% → 5.00% 로 변화가 없었습니다.**
                        #
                        #   이유: 원본 이미지 자체가 이미 밝은 영역의 7~11% 가
                        #   포화인데 모자이크는 5.0% 입니다. 시임·단위 선택이
                        #   **이미** 덜 포화된 프레임을 고르고 있어서, 짜낼
                        #   여지가 대부분 소진된 상태였습니다.
                        #
                        #   남은 포화는 동지 무렵(2025-12-17) 태양 고도 약 29°
                        #   에서 패널 경사와 맞물려 **여러 프레임에서 동시에**
                        #   거울반사가 성립하는 자리라, 어느 프레임을 골라도
                        #   피할 수 없습니다.
                        #
                        #   그래서 기본값을 0(끔)으로 둡니다. 태양 조건이
                        #   다른 촬영에서는 도움이 될 수 있으므로 옵션은 남깁니다.
                        gl = _unit_glint(frames[int(idx)], images,
                                         int(idx), x_min, y_max, g,
                                         u0, v0, u1, v1, glint_thresh,
                                         img_scale)
                        if gl is not None:
                            cost += glint_weight * gl
                    if cost < best_c:
