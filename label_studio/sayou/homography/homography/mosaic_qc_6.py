...
def _panel_edge_steps(panel: np.ndarray, gsd: float) -> dict:
    ...
    if d.size < 200:
        return {}
    res = {
        "edge_step_p99_m": float(np.percentile(d, 99) * gsd),
        "edge_step_max_m": float(d.max() * gsd),
        "edge_step_over3px_ratio": float((d > 3).mean()),
        "edge_step_total_px": int(d.sum()),
    }
    logger.info("  패널 경계 단차: 99%% %.2f m, 최대 %.2f m, 3px 초과 %.1f%% "
                "— 시임이 패널을 자르면 여기서 커집니다 "
                "(--seam-panel-penalty 로 조절)",
                res["edge_step_p99_m"], res["edge_step_max_m"],
