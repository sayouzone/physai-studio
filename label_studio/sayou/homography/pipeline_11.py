...
def run_homography_pipeline(image_dir: Path,
        ...
        if len(tracks) < 10:
            ...
            while True:
                ...
                if (label.startswith("1차") and auto_focal
                    ...
                    if fres is not None:
                        fscale, finfo = fres
                        if abs(fscale - 1.0) > focal_tolerance:
                            f_rep = f_rep / fscale
    ...
    # ---- 7. 지상평면 + 호모그래피 ---------------------------------------
    t0 = time.perf_counter()
    plane, plane_source = _resolve_ground_plane(metas, cams_opt, pts_opt,
    ...
    if mosaic:
        ...
        stats = mosaic_frames(frames, [m.origin_path for m in frame_metas],
                              ...
                              cfg=MosaicConfig(
                                  ...
                                  offnadir_target_fallback=offnadir_target_fallback,
                                  dsm=dsm_obj))
    else:
        n_ok = 0
...
def _resolve_ground_plane(metas, cams_opt, pts_opt, allow_tilted_plane=True,
    ...
    if pts_opt is not None and len(pts_opt) >= 3:
        ...
        if ref_z is not None:
            ...
            if abs(gap) <= 5.0:
                logger.info("기준면 출처: BA 점군 (inlier %d개, LRF 대비 %+.2f m)",
                            plane.n_inliers, gap)
                return plane, "ba_points"
            if abs(gap) > 5.0:
                logger.warning(
    ...
    if zs:
        ...
        return GroundPlane.horizontal(z), "lrf_metadata"
