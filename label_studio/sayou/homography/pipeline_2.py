...
def run_homography_pipeline(image_dir: Path,
    ...
    plane_surface : ``"upper"`` (기본) 는 점군에 두 층이 있으면 위층(패널
        ...
        ``"dominant"``.
    auto_plane : ``True`` (기본) 면 겹침 시차로 기준면 높이를 **데이터에서
        직접** 보정한다. ``panel_top_offset_m`` 을 손으로 맞출 필요가
        없어진다. LRF 는 "레이저가 맞은 지점"의 고도라 패널 상면일 수도
        지면일 수도 있어서, 오프셋을 추측으로 주면 어긋난다. 자동보정은
        그 추측을 제거한다.
    ...
    # ---- 7b. 겹침 시차로 기준면 높이 자동보정 ----------------------------
    if auto_plane and len(frames) >= 4:
        from .homography.plane_calib import calibrate_plane
 
        keep = [(c, m) for c, m in zip(frames, frame_metas)]
 
        def _rebuild(pl):
            out = []
            for fh, m in keep:
                out.append(build_frame_homography(fh.camera_xyz, fh.R,
                                                  fh.intr, pl))
            return out
        ...
        if cal is not None and abs(plane_new.c - plane.c) > 1e-6:
            ...
            frame_metas = [m for f, (_, m) in zip(rebuilt, keep)
                           if f is not None]
