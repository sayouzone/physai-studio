...
def run_homography_pipeline(image_dir: Path,
    ...
    #
    #   실측(EWP-서오창IC-2 IR): 909장 중 tie point 없는 프레임이 25장이고,
    #   유효영역 중심으로 갈수록 패널 면적비가 39.2% → 24.1% 로 떨어졌다
    #   (= 중앙에서 패널이 지면으로 대체됨). BA 자체는 RMSE 0.74 px 로
    #   건강하므로, 원인은 '보정 못 받은 소수 프레임' 쪽이 유력하다.
