...
def run_homography_pipeline(image_dir: Path,
    ...
    # ★ 2층을 쓸지는 **센서에 따라 다릅니다.** 실측(그린환경센터)에서
    #   같은 QC 코드로 잰 결과:
    ...
    #
    #   이유: 2층 경계는 **단일 평면 정사영상**에서 따므로 이미 Δh·k 만큼
    #   밀려 있습니다. 그 밀림이 GSD 대비 몇 픽셀인지가 톱니의 크기입니다.
    #     RGB(GSD 0.64cm): k=0.15 에서 23 px, k=0.30 에서 47 px → 눈에 보임
    #     IR (GSD 4.15cm): k=0.15 에서 3.6 px, k=0.30 에서 7.2 px → 부화소급
    #   즉 GSD 가 거칠수록 2층의 손해가 묻히고 이득(층 높이차 1 m)만 남습니다.
    #
    #   그래서 '경계 밀림이 몇 픽셀인가' 로 자동 판정합니다.
    if use_two_layer and two_layer_auto and dsm_obj is None:
        # 실제 연직 상한: --offnadir-frac 이 자동 결정에 쓰이므로
        # 프레임 최대 k 의 그 비율을 대표값으로 삼는다.
        _k_typ = max(float(max_offnadir_ratio), 0.05)
        _dh = max(float(_ground_drop_hint), 0.3)
        _shift_px = _dh * _k_typ / max(gsd_m or 0.01, 1e-6)
        if _shift_px > two_layer_max_shift_px:
            logger.info(
                "2층 표면 자동 해제: 층 경계가 GSD 기준 %.0f px 밀려 있어 "
                "(층 높이차 %.2f m × k %.2f ÷ GSD %.4f m) 톱니 손해가 "
                "이득을 넘어섭니다. 한계 %.0f px. "
                "GSD 가 거친 IR 에서는 자동으로 켜집니다.",
                _shift_px, _dh, _k_typ, gsd_m, two_layer_max_shift_px)
            use_two_layer = False
        else:
