...
from tasks.models import Annotation, AnnotationDraft, Prediction, PredictionMeta, Task, TaskGeoreferencing
...
class _GeoJSONPolygonField(serializers.Field):
    """PolygonField -> GeoJSON dict (읽기 전용)."""
...
class TaskGeoreferencingSerializer(serializers.ModelSerializer):
    """TaskGeoreferencing 전체 직렬화.
 
    기존 GeoreferencingSerializer 대비: 필드 슈퍼셋 + 범위 검증 +
    footprint GeoJSON. 기존 클라이언트가 쓰던 필드는 모두 동일 이름 유지.
    """
 
    footprint_rgb = _GeoJSONPolygonField()
    footprint_ir = _GeoJSONPolygonField()
    footprint_rgb_corrected = _GeoJSONPolygonField()
    footprint_ir_corrected = _GeoJSONPolygonField()
 
    # 파생 프로퍼티 노출 (읽기 전용)
    is_rtk_fixed = serializers.BooleanField(read_only=True)
    is_ready = serializers.BooleanField(read_only=True)
    camera_height_m = serializers.FloatField(read_only=True, allow_null=True)
 
    class Meta:
        model = TaskGeoreferencing
        fields = [
            'id', 'task',
            # 위치/자세
            'latitude', 'longitude', 'altitude', 'relative_height',
            'yaw', 'pitch', 'roll', 'flight_yaw',
            # 지면고도
            'ground_alt', 'ground_alt_source',
            # RTK 품질
            'rtk_flag', 'rtk_std_lat', 'rtk_std_lon', 'rtk_std_hgt',
            'is_rtk_fixed',
            # 카메라/촬영
            'captured_at', 'camera_model', 'modality',
            'focal_length_mm', 'focal_length_35mm',
            'image_width', 'image_height', 'image_sha1',
            # georeferencing 결과 (기존 호환)
            'ground_sample_distance', 'method', 'accuracy_m',
            'offset_correction', 'transform_matrix', 'raw_metadata',
            # footprint (파생, 읽기 전용)
            'footprint_rgb', 'footprint_ir',
            'footprint_rgb_corrected', 'footprint_ir_corrected',
            'footprint_low_confidence',
            # 파이프라인
            'status', 'extractor_version', 'error_message',
            'is_ready', 'camera_height_m',
    ...
    def validate_pitch(self, v):
        if v is not None and not (-180.0 <= v <= 180.0):
            raise serializers.ValidationError('pitch는 -180~180 범위여야 합니다.')
        return v
 
    def validate_yaw(self, v):
        if v is not None and not (-360.0 <= v <= 360.0):
            raise serializers.ValidationError('yaw는 -360~360 범위여야 합니다.')
        return v
 
    def validate_rtk_flag(self, v):
        if v is not None and v not in (0, 16, 34, 50):
            raise serializers.ValidationError(
                'rtk_flag는 DJI 값(0, 16, 34, 50) 중 하나여야 합니다.')
        return v
 
    def validate_modality(self, v):
        if v and v not in ('wide', 'zoom', 'ir', 'screen'):
            raise serializers.ValidationError(
                "modality는 wide | zoom | ir | screen 중 하나여야 합니다.")
        return v
 
    def validate(self, attrs):
        # 좌표는 쌍으로만 갱신 (한쪽만 바뀌면 위치가 붕괴)
        lat_in = 'latitude' in attrs
        lng_in = 'longitude' in attrs
        if lat_in != lng_in:
            raise serializers.ValidationError(
                'latitude와 longitude는 함께 제공해야 합니다.')
        return attrs
