...
from projects.serializers import (
    ...
    ProjectGeoreferencingSerializer,
)
...
from tasks.models import Annotation, Task, TaskGeoreferencing
...
def extract_project_georeferencing_job(project_id: int,
                                       only_pending: bool = True) -> dict:
    """프로젝트 전체 task 일괄 추출."""
    from tasks.georeferencing import extract_georeferencing_job, EXTRACTOR_VERSION

    qs = Task.objects.filter(project_id=project_id).values_list('id', flat=True)
    processed, failed = 0, 0
    for task_id in qs.iterator(chunk_size=500):
        if only_pending:
            existing = TaskGeoreferencing.objects.filter(
                task_id=task_id).first()
            if existing and existing.status == \
                    TaskGeoreferencing.Status.COMPLETED \
        ...
        if result['status'] == 'done':
            ...
            failed += 1
    logger.info('[georef-extract] project=%s processed=%d failed=%d',
                project_id, processed, failed)
    return {'status': 'done', 'project': project_id,
...
@method_decorator(
    ...
    decorator=extend_schema(
        tags=['Georeferencing'],
        summary='Batch-extract georeferencing for a project',
        description='Run XMP+EXIF extraction for all tasks in the project. '
        'By default skips tasks already completed with the current extractor '
        'version; pass force=true to recompute everything.',
        parameters=[
            OpenApiParameter(name='pk', type=OpenApiTypes.INT,
                             location='path', description='Project ID'),
        ],
        request=None,
        responses={
            '200': OpenApiResponse(
                description='Batch result (sync) or accepted (async)',
                response=ProjectGeoreferencingSerializer,
            )
        },
        extensions={
            'x-fern-sdk-group-name': 'georeferencing',
            'x-fern-sdk-method-name': 'batch_extract',
            'x-fern-audiences': ['public'],
        },
    ),
)
class ProjectGeoreferencingAPI(generics.RetrieveAPIView):
    """GET api/projects/<pk>/georeferencing/"""
    ...
    def get(self, request, pk):
        ...
        only_pending = request.GET.get('force') != 'true'
        result = start_job_async_or_sync(
            extract_project_georeferencing_job, project.id,
            only_pending=only_pending)
        if isinstance(result, dict):
            return Response(result)
        return Response({'status': 'queued', 'project': project.id},
