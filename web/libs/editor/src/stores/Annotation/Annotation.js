...
const _Annotation = types
  ...
  .actions((self) => ({
    ...
    duplicateRegion(region) {
      ...
      // ✅ Area가 아니라 region의 results를 각각 serialize
      //    (Annotation.serialized와 동일한 방식)
      const serializedResults = region.results
        .map((r) => r.serialize())
        .filter(Boolean);

      if (!serializedResults.length) {
        console.warn("duplicateRegion: no serializable results");
        return null;
      }

      // ✅ 같은 region의 result들은 같은 id를 공유해야
      //    appendResults에서 하나의 area로 묶임
      //    (appendResults가 regionIdMap으로 id를 새로 생성하므로
      //     여기서는 기존 id를 유지해도 됨 — 단, 전부 동일해야 함)
      const OFFSET = 2;

      const duplicated = serializedResults.map((s) => {
        const copy = structuredClone(s);

        // 좌표 오프셋 (겹침 방지) — bbox류만
        if (typeof copy.value?.x === "number") {
          copy.value.x = Math.min(copy.value.x + OFFSET, 100 - (copy.value.width ?? 0));
          copy.value.y = Math.min(copy.value.y + OFFSET, 100 - (copy.value.height ?? 0));
        }
      ...
      // ✅ this → self
      const newRegions = self.appendResults(duplicated);
      const newRegion = newRegions?.[0];

      if (newRegion) {
        self.selectArea(newRegion);
      }
