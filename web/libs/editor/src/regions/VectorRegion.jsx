 ...
 *
 * Handles conversion between PhysAI Studio's relative coordinates (0-100%) and KonvaVector's image coordinates (pixels).
 *
 * **Coordinate Systems:**
 * - **PhysAI Studio**: Relative coordinates (percentages) for storage and portability
 * - **KonvaVector**: Image coordinates (pixels) for precise editing
 *
 * **Conversion Flow:**
 * ```
 * PhysAI Studio (Relative %) ←→ VectorRegion ←→ KonvaVector (Image px)
 ...
 * - `serialize()`: Exports in PhysAI Studio format with relative coordinates
 */
const Model = types
  .model({
  ...
  .actions((self) => {
    return {
      /**
       * Converts coordinates from PhysAI Studio format (relative %) to KonvaVector format (image pixels)
      ...
      /**
       * Serializes region data in PhysAI Studio format with relative coordinates
       ...
       *
       * @return {VectorRegionResult} The serialized vector region data in PhysAI Studio format
       */
      serialize() {
