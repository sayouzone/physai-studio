...
export function PersonalJWTToken() {
  ...
  const revoke = useCallback(
    async (token: string) => {
      confirm({
        title: "Revoke Token",
        body: `Are you sure you want to delete this access token? Any application using this token will need a new token to be able to access ${window?.APP_SETTINGS?.app_name || "PhysAI Studio"
          }`,
        okText: "Revoke",
        buttonLook: "negative",
        onOk: async () => {
